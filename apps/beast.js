...
sorto=function(e){
  ...
  var lower=1;
  if (target.getAttribute("class")=="sbtnu"){lower=0;}
  ...
  document.getElementById("debug1").innerText=lower;
  if (numa==0){
  	myfunct=function(a, b){
    	return (a[ptr]>b[ptr])?lower:((lower-1)*(-1));
    }
  }
  else{
    myfunct=function(a, b){
      return (parseInt(a[ptr])>parseInt(b[ptr]))?lower:((lower-1)*(-1));
    }
  }
  document.getElementById("debug1").innerText=x; 
  x.sort(myfunct);
